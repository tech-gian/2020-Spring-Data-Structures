...
void test_pq_sort_vector(void) {
	...
	int N = 10;
	for (int i = 0; i < N; i++)
		vector_insert_last(vec, create_int_test(rand()));
...
void test_pq_sort_list(void) {
	...
	int N = 10;
	for (int i=0 ; i<N ; i++) {
		list_insert_next(list, LIST_BOF, create_int_test(rand()));
	}

	// Sort
	pq_sort_list(list, compare_ints);

	// Έλεγχος ότι είναι οι σωστές τιμές
	int last = INT_MIN;
