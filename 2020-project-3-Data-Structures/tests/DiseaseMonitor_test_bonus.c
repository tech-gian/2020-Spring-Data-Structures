...
// 
// Νέα tests που δημιουργήθηκαν για το bonus ερώτημα της
// Άσκησης 3, της Εργασίας 3
//
//////////////////////////////////////////////////////////////////
...
// Χρησιμοποιούνται οι ίδιες εγγραφές για το DiseaseMonitor
...
// Θεώρησα σκόπιμο, για την πληρότητα του test, να διατηρηθούν οι έλεγχοι
// που γίνονται ήδη (στο test που μας δόθηκε έτοιμο) και απλά να
// προσθέσω όσους ακόμα θεωρώ αναγκαίους.
...
void test_init(void) {
	...
	// Προσθέτουμε 1 εγγραφή και ελέγχουμε ότι έγινε εισαγωγή
    dm_insert_record(&records[0]);
    list = dm_get_records(NULL, NULL, NULL, NULL);
    TEST_ASSERT(list_size(list) == 1);
	list_destroy(list);


	// Ξανα-αρχικοποιούμε το dm, ώστε να ελεγχθεί ότι γίνεται destroy
	// το προηγούμενο και δημιουργείται νέο, που είναι άδειο
    dm_init();
    list = dm_get_records(NULL, NULL, NULL, NULL);
    TEST_ASSERT(list_size(list) == 0);
    list_destroy(list);
...
void test_get_records(void) {
	...
	// Έλεγχος για τα κριτήρια που δίνονται και στο αρχικό test
	...
	// Έλεγχος με νέα κριτήρια
	// (κυρίως έλεγχος για κενή λίστα-επιστροφής εγγραφών, ακραίες
	// αλλά πιθανές περιπτώσεις)

    List list = dm_get_records("abcd", "Lannister", "0299-01-01", "0301-01-01");
    int ids[] = {};
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

    list = dm_get_records("Grayscale", "abcd", "0299-01-01", "0300-01-01");
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

    list = dm_get_records("abcd", "Lannister", "abcd", "0301-01-01");
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

    list = dm_get_records("abcd", "Lannister", "0299-01-01", "abcd");
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

    list = dm_get_records("Grayscale", "abcd", "abcd", "0301-01-01");
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

    list = dm_get_records("Grayscale", "abcd", "0299-01-01", "abcd");
    check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records("abcd", "Lannister", NULL, NULL);
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records("Pale Mare", "Targaryen", "abcd", "abcd");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records("Grayscale", "abcd", NULL, NULL);
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records(NULL, NULL, "abcd", "abcd");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records(NULL, "Lannister", "abcd", "abcd");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records("Grayscale", NULL, "abcd", "abcd");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records("abcd", NULL, "0299-01-01", "0300-01-01");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));

	list = dm_get_records(NULL, "abcd", "0299-01-01", "0300-01-01");
	check_record_list(list, ids, sizeof(ids)/sizeof(int));
...
void test_count_records(void) {
	...
	// Έλεγχος για νέα (τα ίδια με την test_get_records) κριτήρια
	// (ελέγχονται πάλι ακραίες αλλά πιθανές περιπτώσεις)

	count_and_test("abcd", "Lannister", "0299-01-01", "0301-01-01");
	count_and_test("Grayscale", "abcd", "0299-01-01", "0300-01-01");
	count_and_test("abcd", "Lannister", "abcd", "0301-01-01");
	count_and_test("abcd", "Lannister", "0299-01-01", "abcd");
	count_and_test("Grayscale", "abcd", "abcd", "0301-01-01");
	count_and_test("Grayscale", "abcd", "0299-01-01", "abcd");
	count_and_test("abcd", "Lannister", NULL, NULL);
	count_and_test("Pale Mare", "Targaryen", "abcd", "abcd");
	count_and_test("Grayscale", "abcd", NULL, NULL);
	count_and_test(NULL, NULL, "abcd", "abcd");
	count_and_test(NULL, "Lannister", "abcd", "abcd");
	count_and_test("Grayscale", NULL, "abcd", "abcd");
	count_and_test("abcd", NULL, "0299-01-01", "0300-01-01");
	count_and_test(NULL, "abcd", "0299-01-01", "0300-01-01");
...
void test_top_diseases(void) {
	...
	// Έλεγχος για νέα κριτήρια, όπως η country == "Lannister"
	// και επίσης για 1 χώρα που δεν υπάρχει
	// (έλεγχος ότι επιστρέφερει άδεια λίστα με τις top_diseases)
	

	run_and_test_top_diseases(1, "Lannister");

	run_and_test_top_diseases(0, "abcd");
