...
Pointer list_get_at(List list, int pos) {
	// Παίρνουμε τον πρώτο κόμβο
	ListNode node = list_first(list);
	// Φτάνουμε μέχρι την θέση pos
	while (pos--) {
...
void list_remove(List list, ListNode node) {
	// Ελέγχουμε αν ο node είναι ο πρώτος κόμβος της λίστας
	if (node==list_first(list)) {
		list_remove_next(list, LIST_BOF);
	}
	// Αλλιώς ψάχνουμε να βρούμε τον προηγούμενο του node
	else {
		for (ListNode temp_node=list_first(list);
			temp_node!=LIST_EOF;
			temp_node=list_next(list, temp_node)) {

				if (list_next(list, temp_node)==node) {
					list_remove_next(list, temp_node);
					break;
				}
			}
	}
...
void list_append(List list, List to_append) {
	// Ελέγχουμε αν η to_append είναι κενή
	if (list_size(to_append)!=0) {
		
		// Ενημερώνουμε το size του list
		list->size += list_size(to_append);		
	
		// Βάζουμε τον πρώτο κόμβο της to_append
		// μετά τον τελευταίο της list
		list->last->next = list_first(to_append);

		// Ενημερώνουμε το last της list
