...
struct vector {
	...
	int steps;					// Πόσα steps έχουν γίνει στην τελευταία συνάρηση που κλήθηκε
};
...
Vector vector_create(int size, DestroyFunc destroy_value) {
	...
	// Η calloc κάνει τόσα βήματα όσα και το capacity, αφού αρχικοποιεί σε 0
	vec->steps = vec->capacity;
	return vec;
}
...
void vector_insert_last(Vector vec, Pointer value) {
	...
	if (vec->capacity == vec->size) {
		// Άμα γίνει realloc, θα γίνουν τόσα βήματα, όσα και το size του vector
		vec->steps = vec->size;
...
Pointer vector_find(Vector vec, Pointer value, CompareFunc compare) {
	...
	for (int i = 0; i < vec->size; i++) {
		// Μετράμε ακριβώς πόσα βήματα θα γίνουν
		vec->steps++;
...
// Άσκηση 1 ///////////////////////////////////////////////////////////////////////
// Σε κάθε λειτουργία που έχει σταθερό αριθμό βημάτων, επιστρέφω 1 βήμα
// Ενώ άμα εξαρτάται από το n, επιστρέφω τον ακριβή αριθμών βημάτων
