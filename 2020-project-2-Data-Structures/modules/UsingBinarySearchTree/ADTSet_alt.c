...
// με διπλά συνδεδεμένη λίστα, χρησιμοποιώντας κόμβους
// next, prev σε κάθε κόμβο
//
///////////////////////////////////////////////////////////
...
struct set {
	...
	SetNode first, last;		// Κόμβοι, που δείχνουν στον πρώτο και τελευταίο αντίστοιχα
};

// Ενώ το struct set_node είναι κόμβος ενός Δυαδικού Δέντρου Αναζήτησης
struct set_node {
	SetNode left, right;		// Παιδιά
    SetNode prev, next;			// Κόμβοι που δείχνουν στον προηγούμενο και επόμενο αντίστοιχα
	Pointer value;
};
...
Set set_create(CompareFunc compare, DestroyFunc destroy_value) {
	...
	set->first = NULL;			// δεν υπάρχει πρώτος
	set->last = NULL;			// δεν υπάρχει τελευταίος
	set->size = 0;
	set->compare = compare;
...
void set_insert(Set set, Pointer value) {
	...
	// Βρίσκω τον κόμβο και συμπληρώνω τον prev και next του
	SetNode target = node_find_equal(set->root, set->compare, value);
	target->prev = node_find_previous(set->root, set->compare, target);
	target->next = node_find_next(set->root, set->compare, target);


	// Ανάλογα τι έχει επιστραφεί σαν prev, next
	// κάνω τις απαραίτητες αλλαγές
	if (target->prev != NULL)
...
bool set_remove(Set set, Pointer value) {
	...
	// Ψάχνω τον κόμβο που πρέπει να διαγραφεί
	SetNode target = node_find_equal(set->root, set->compare, value);

	// Αν βρεθεί κάνω τις απαραίτητες αλλαγές
	if (target != NULL) {
		if (target->prev != SET_BOF)
			(target->prev)->next = target->next;
		else
			set->first = target->next;
		
		if (target->next != SET_EOF)
