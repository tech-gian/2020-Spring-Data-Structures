...
struct set {
	...
	BList blist;				// BList, για γρήγορες λειτουργίες διάσχησης του Set
...
struct set_node {
	...
	Pointer value;				// Τιμή value
	BListNode blist_node;		// Κόμβος της BList
};
...
Set set_create(CompareFunc compare, DestroyFunc destroy_value) {
	...
	set->blist = blist_create(NULL);	// Δημιουργία BList, χωρίς destroy_value
...
void set_insert(Set set, Pointer value) {
	...
	// Βρίσκω το SetNode που έγινε inserted
	SetNode node = node_find_equal(set->root, set->compare, value);
	// Βρίσκω και το επόμενο του
	SetNode next_node = node_find_next(set->root, set->compare, node);

	// Αν είναι == NULL, τότε βάζω το BListNode στο τέλος
	if (next_node == NULL) {
		blist_insert(set->blist, BLIST_EOF, node);
		node->blist_node = blist_last(set->blist);
	}
	// Αλλιώς το βάζω πριν από το next_node
	else {
		blist_insert(set->blist, next_node->blist_node, node);
		node->blist_node = blist_previous(set->blist, next_node->blist_node);
	}
...
bool set_remove(Set set, Pointer value) {
	...
	// Βρίσκω τον κόμβο με τιμή value
	SetNode node = node_find_equal(set->root, set->compare, value);
	
	// Αν υπάρχει, αφαιρώ το BListNode
	if (node != NULL)
		blist_remove(set->blist, node->blist_node);

	// Και μετά αφαιρώ και το SetNode
...
SetNode set_first(Set set) {
	// Επιστροφή του value του πρώτου BListNode (δηλαδή ενός SetNode)
	return (SetNode) blist_node_value(set->blist, blist_first(set->blist));
}

SetNode set_last(Set set) {
	// Επιστροφή του value του τελευταίου BListNode (δηλαδή ενός SetNode)
	return (SetNode) blist_node_value(set->blist, blist_last(set->blist));
}

SetNode set_previous(Set set, SetNode node) {
	// Βρίσκω το BListNode του node
	BListNode bnode = node->blist_node;

	// Αν το προηγούμενο του είναι NULL
	if (bnode == blist_first(set->blist))
		return SET_BOF;
	// Σε διαφορετική περίπτωση
	else
		return (SetNode) blist_node_value(set->blist, blist_previous(set->blist, bnode));
}
...
SetNode set_next(Set set, SetNode node) {
	...
	// Αν το επόμενο του είναι NULL
	if (bnode == blist_last(set->blist))
		return SET_EOF;
	// Σε διαφορετική περίπτωση
	else
		return (SetNode) blist_node_value(set->blist, blist_next(set->blist, bnode));
}
