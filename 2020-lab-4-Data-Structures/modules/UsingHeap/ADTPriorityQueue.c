...
// Αρχικοποιεί το σωρό από τα στοιχεία του vector values. (ΜΗ αποδοτικό)
...
static void efficient_heapify(PriorityQueue pqueue, Vector values) {	
	// Περνάω τις τιμές του values στο vector της pqueue
	int size = vector_size(values);
	for (int i= 0 ; i<size ; i++) {
		vector_insert_last(pqueue->vector, vector_get_at(values, i));
	}

	// Ξεκινώντας από την τελευταία σειρά που έχει γονιούς
	// φτάνω μέχρι την κορυφή και κάνω bubble_down σε κάθε κόμβο
	// (το size/2 είναι ο "χαμηλότερος" και "δεξιότερος" γονιός)
