...
// Βιβλιοθήκη για την συνάρτηση clock()
#include "time.h"
...
void test_visit() {
	...
	// Μέτρηση της set_visit με την clock()
	double start = ((double)clock())/CLOCKS_PER_SEC;

	set_visit(set, visit);

	double end = ((double)clock())/CLOCKS_PER_SEC;
	printf("time with clock(): %.8f", end-start);

	TEST_ASSERT(last_visited == N-1);
	set_destroy(set);
}

void test_visit_efficient(void) {
	// Δοκιμή της set_visit σε ένα set με N στοιχεία
	Set set = set_create(compare_ints, free);
...
TEST_LIST = {
	...
	{ "set_visit_efficient", test_visit_efficient },
