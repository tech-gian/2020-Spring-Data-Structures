...
static void rec(BTreeNode node, VisitFunc visit) {
	for (int i=0 ; i<node->count+1 ; i++) {
		// Καλώ την rec για κάθε παιδί
		if (node->children[i] != NULL)
			rec(node->children[i], visit);
		// Αν δεν έχει παιδί, καλώ την visit για
		// όλα τα value του κόμβου
		else {
			for (int j=0 ; j<node->count ; j++) {
				visit(node->set_nodes[j]->value);
			}
			return;
		}
		
		// Αν δεν βρίσκομαι στον τελευταίο i (τελευταίο παιδί)
		// καλώ την visit για το value του κόμβου
		if (i != node->count)
